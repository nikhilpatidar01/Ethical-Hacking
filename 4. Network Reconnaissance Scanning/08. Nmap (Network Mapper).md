...
```
nmap 192.168.1.35                   # Scan specific IP  
nmap target.com                    # Scan hostname/domain  
nmap -v 192.168.1.35               # Verbose scan  
nmap -v scanme.nmap.org           # Scan authorized test server  
```
...
| Field            | Usage                                             |
| ---------------- | ------------------------------------------------- |
| IP               | OS detection and hop counting (`--ttl`)           |
| Flags            | Fragmentation and stealth scans (`-f`)            |
| Source IP        | IP spoofing (`-S`)                                |
| IP Protocol      | Protocol scan (`-sO`)                             |
| TCP Flags        | Used for various scan types (SYN, FIN, ACK, etc.) |
| Source Port      | Firewall evasion (`--source-port`)                |
| Destination Port | Defines target port (`-p`)                        |
| TCP Options      | Craft custom TCP packets (`--tcp-options`)        |
| ICMP Type/Code   | Ping/traceroute (Types 0, 3, 8, 11)               |
...
If you want this saved as a **PDF** or formatted as a **GitHub README**, just let me know.
